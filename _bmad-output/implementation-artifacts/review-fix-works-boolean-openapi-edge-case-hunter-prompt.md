...
Use the `bmad-review-edge-case-hunter` skill. Treat the diff below as the strict review scope; repository access is allowed only to resolve symbols directly referenced by changed lines. Return only the JSON array required by that skill.
